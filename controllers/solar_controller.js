...
const data_extraction_sum = async (req, res) => {
  ...
  for (let i = 0; i < newArray.length; i++) {
    ...
    initial_prod = Math.round((initial_prod + Number(newArray[i].E_m))*100)/100;
  }

  // Calculate annual production including panel degration
  var annual_prod = new Array(system_life);
  for (let i = 0; i < system_life; i++) {
    annual_prod[i] = Math.round(initial_prod*((1-panel_degradation_rate)**i)*100)/100
  }
  ...
  for (let i = 0; i < system_life; i++) {
    elec_day_proj[i] = Math.round((current_day_elec_price*(1+inflation_rate+elec_price_increase_rate)**(i))*1000)/1000;
    elec_night_proj[i] = Math.round((current_night_elec_price*(1+inflation_rate+elec_price_increase_rate)**(i))*1000)/1000;
    elec_sale_proj[i] = Math.round((current_elec_sale_price*(1+inflation_rate)**(i))*1000)/1000;
  }
  ...
  for (let i=0; i<(system_life); i++){ 
    incoming[i] = (annual_prod[i]*self_consumption)*elec_day_proj[i] + (annual_prod[i]*(1-self_consumption))*elec_sale_proj[i];
  }
  ...
  for (let i=0; i<(system_life); i++){ 
    if (i == 0){
      ...
      simple_cumulative[i] = Math.round(simple_cumulative[i-1]+incoming[i]-outgoing[i]);
    }
  }
  for (let i=0; i<(system_life); i++){ 
  ...
  for (let i=0; i<(system_life); i++){ 
    ...
    net_present_value = Math.round(net_present_value - discount_outgoing[i] + discount_incoming[i])
  }
  ...
  const json_calc_results = {
    "monthly_production":JSON.stringify(monthly_production),
    "annual_production":JSON.stringify(annual_prod),
    "simple_cumulative":JSON.stringify(simple_cumulative),
    "simple_payback":JSON.stringify(simple_payback),
    "net_present_value":JSON.stringify(net_present_value),
  }
